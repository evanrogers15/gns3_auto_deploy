...
configure_mgmt_tap = app_config.configure_mgmt_tap
...
def main():
    ...
    network_test_tool_deploy_data = {"x": -107, "y": 570, "name": "Test-Client"}
    deploy_data_z = {"z": -1}
    ...
    def gns3_get_node_files(project_id, node_id, file_path):
        for server_record in gns3_server_data:
            ...
            try:
                ...
                exit()
            return nodes
    def gns3_restart_service():
        for server_record in gns3_server_data:
            server_ip, server_port = server_record['GNS3 Server'], server_record['Server Port']
            node_url = f"http://{server_ip}:{server_port}/v2/shutdown"
            response = make_request("POST", node_url, data={})
            print(response)
            #if response.status_code == 200:
            #    print(f"[{util_current_time()}] - Restarted GNS3 Service")
            #else:
            #    print(f"[{util_current_time()}] - Failed to restart GNS3 service: {response.content}")
