...
import sqlite3
from modules.use_cases import use_case_1, use_case_2, use_case_3, use_case_4, use_case_4_sim_user
from modules.gns3_query import get_projects, get_computes_name
from modules.gns3_actions_old import reset_lab_client_states
import json
import subprocess
import psutil
import requests
...
def update_uc_config():
    ...
    row = c.fetchone()
    if row[0] == 0:
        c.execute("INSERT INTO uc_config (server_ip, server_port, server_name, project_list, project_name, project_status) VALUES (?, ?, ?, ?, ?, ?)", (server_ip, server_port, server_name, json.dumps(project_ids), json.dumps(project_names), json.dumps(project_status)))
    else:
        c.execute("UPDATE uc_config SET server_port = ?, server_name = ?, project_list = ?, project_name = ?, project_status = ? WHERE server_ip = ?", (server_port, server_name, json.dumps(project_ids), json.dumps(project_names), json.dumps(project_status), server_ip))
    # Insert data into the uc_projects table if project_id is present
    if 'project_id' in req_data:
        project_id = req_data['project_id']
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT project_list FROM uc_config WHERE server_ip = ? AND server_port = ?", (server_ip, server_port))
        row = c.fetchone()
        if row:
            project_list = json.loads(row[0])
            if project_id in project_list:
                index = project_list.index(project_id)
                c.execute("SELECT project_name, project_status FROM uc_config WHERE server_ip = ? AND server_port = ?", (server_ip, server_port))
                row = c.fetchone()
                if row:
                    ...
                    row = c.fetchone()
                    if row[0] == 0:
                        c.execute("INSERT INTO uc_projects (server_name, server_ip, server_port, project_id, project_name, project_status) VALUES (?, ?, ?, ?, ?, ?)", (req_data.get('server_name'), server_ip, server_port, project_id, project_name, project_status))
    conn.commit()
    conn.close()
...
def get_project_list():
    ...
    server_ip = row[0]
    server_port = row[1]
    c.execute("SELECT project_list, project_name, project_status FROM uc_config WHERE server_ip=? AND server_port=?", (server_ip, server_port))
    row = c.fetchone()
    if row is None:
        conn.close()
...
def uc_scenarios():
    ...
    if request.method == 'GET':
        ...
        if scenario_id is None:
            c.execute("INSERT INTO uc_scenarios (scenario_name, scenario_description) VALUES (?, ?)", (scenario_name, scenario_description))
        ...
        # If scenario_id is provided, update an existing scenario
            ...
            conn.commit()
        # Return the scenario
        c.execute("SELECT id, scenario_name AS title, scenario_description AS description FROM uc_scenarios WHERE id=?", (scenario_id,))
        row = c.fetchone()
...
def create_task(scenario_id):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT COUNT(*) FROM uc_scenario_status WHERE server_ip=? AND project_id=? AND scenario_id=?", (server_ip, project_id, scenario_id,))
    row = c.fetchone()
    if row[0] == 0:
        c.execute("INSERT INTO uc_scenario_status (server_ip, project_id, scenario_id, status) VALUES (?, ?, ?, ?)", (server_ip, project_id, scenario_id, 1))
    else:
        c.execute("UPDATE uc_scenario_status SET status=? WHERE server_ip=? AND project_id=? AND scenario_id=?", (1, server_ip, project_id, scenario_id))
    conn.commit()
    conn.close()
    ...
    if scenario_id == 1:
        ...
        # Update the scenario status table with the process ID
            ...
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("UPDATE uc_scenario_status SET status=?, process_id=? WHERE server_ip=? AND project_id=? AND scenario_id=?", (2, pid, server_ip, project_id, scenario_id))
            conn.commit()
            conn.close()
    elif scenario_id == 3:
        # Set the payload for the request
    ...
    if use_case_function is not None:
        ...
        if success:
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("UPDATE uc_scenario_status SET status=? WHERE server_ip=? AND project_id=? AND scenario_id=?", (2, server_ip, project_id, scenario_id))
            conn.commit()
            conn.close()
    scenario = {
        'running': True
...
def delete_task(scenario_id):
    ...
    row = c.fetchone()
    if row[0] == 0:
        c.execute("INSERT INTO uc_scenario_status (server_ip, project_id, scenario_id, status) VALUES (?, ?, ?, ?)", (server_ip, project_id, scenario_id, status))
    else:
        c.execute("UPDATE uc_scenario_status SET status=? WHERE server_ip=? AND project_id=? AND scenario_id=?", (status, server_ip, project_id, scenario_id))
    conn.commit()
    conn.close()
    # Call the appropriate use case function based on the scenario_id
    if scenario_id == 1:
...
def stop_script():
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT process_id FROM uc_scenario_status WHERE project_id=? AND scenario_id=?", (project_id, scenario_id,))
    row = c.fetchone()
    if row is None:
        return jsonify({'error': 'Scenario not found.'}), 404
    process_id = row[0]
    ...
    try:
        ...
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("UPDATE uc_scenario_status SET process_id = NULL WHERE project_id = ? AND scenario_id = ?", (project_id, scenario_id,))
        conn.commit()
        conn.close()
