...
def viptela_8000v_appneta_deploy():
    ...
    # endregion
    # region Viptela cEdge Device Setup
    deployment_step = 'cEdge Device Setup'
    log_and_update_db(server_name, project_name, deployment_type, deployment_status, deployment_step, f"Starting cEdge Device Setup for {site_count} cEdges")
    server_ips = set(d['GNS3 Server'] for d in gns3_server_data)
    abs_path = os.path.abspath(__file__)
    configs_path = os.path.join(os.path.dirname(abs_path), '../configs/viptela')
        ...
        for i in range(1, site_count + 1):
            ...
            if matching_nodes:
                for matching_node in matching_nodes:
                    ...
                    tn.read_until(b"exit")
                    tn.close()
                    log_and_update_db(server_name, project_name, deployment_type, deployment_status, deployment_step, f"Completed cEdge Device Setup for {temp_node_name}, Remaining - {site_count - i}")
                    if i % 44 == 0 and i != 0:
                        isp_index += 1
    log_and_update_db(server_name, project_name, deployment_type, deployment_status, deployment_step, f"Completed cEdge Device Setup for {site_count} cEdge devices")
    # endregion
