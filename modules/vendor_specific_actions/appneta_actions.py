...
def appneta_cli_curl_commands(server_ip, project_name, deployment_type, console_port, node_name, appn_password, mp_ip_address, appn_site_key, appn_url, mp_lan_address=None, mp_lan_gateway=None):
    ...
    log_and_update_db(server_ip, project_name, deployment_type, 'Running', deployment_step, f"Starting configuration on {node_name}")
    tn.write(b"\r\n")
    while True:
        tn.write(b"\r\n")
    ...
    tn.write(appn_password.encode("ascii") + b"\n")
    log_and_update_db(server_ip, project_name, deployment_type, 'Running', deployment_step,
