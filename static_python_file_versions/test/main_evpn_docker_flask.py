...
def gns3_connect_nodes(project_id, node_a, adapter_a, port_a, node_b, adapter_b, port_b):
    for server_record in gns3_server_data:
        ...
        node_a_name = gns3_find_nodes_by_field(project_id, 'node_id', 'name', node_a)
        node_b_name = gns3_find_nodes_by_field(project_id, 'node_id', 'name', node_b)
        print(
...
def gns3_delete_nodes(project_id):
    for server_record in gns3_server_data:
        server_ip, server_port = server_record['GNS3 Server'], server_record['Server Port']
...
def gns3_find_nodes_by_name(project_id, search_string=None):
    node_data = gns3_get_nodes(project_id)
    if search_string:
...
def gns3_get_location_data(project_id, item_type):
    for server_record in gns3_server_data:
        server_ip, server_port = server_record['GNS3 Server'], server_record['Server Port']
...
def gns3_find_nodes_by_field(project_id, search_field, return_field, search_string):
    nodes = gns3_get_nodes(project_id)
    if search_string:
...
def gns3_reload_node(project_id, node_id):
    for server_record in gns3_server_data:
        server_ip, server_port = server_record['GNS3 Server'], server_record['Server Port']
...
def gns3_upload_file_to_node(project_id, node_id, file_path_var, filename_temp):
    ...
    for server_record in gns3_server_data:
        ...
        node_name = gns3_find_nodes_by_field(project_id, 'node_id', 'name', node_id)
...
def gns3_stop_node(project_id, node_id):
    for server_record in gns3_server_data:
        server_ip, server_port = server_record['GNS3 Server'], server_record['Server Port']
        template_data = {}
...
def vmanage_set_cert_type(vmanage_headers):
    server_ips = set(d['vManage API IP'] for d in gns3_server_data)
    for server_ip in server_ips:
...
def vmanage_set_cert(vmanage_headers, cert):
    server_ips = set(d['vManage API IP'] for d in gns3_server_data)
    for server_ip in server_ips:
...
def vmanage_install_cert(vmanage_headers, cert):
    server_ips = set(d['vManage API IP'] for d in gns3_server_data)
    for server_ip in server_ips:
...
def vmanage_forcesync_rootcert(vmanage_headers):
    server_ips = set(d['vManage API IP'] for d in gns3_server_data)
    for server_ip in server_ips:
...
def vmanage_sync_rootcertchain(vmanage_headers):
    server_ips = set(d['vManage API IP'] for d in gns3_server_data)
    for server_ip in server_ips:
...
def vmanage_set_vbond(vmanage_headers):
    server_ips = set(d['vManage API IP'] for d in gns3_server_data)
    for server_ip in server_ips:
...
def vmanage_set_org(vmanage_headers):
    server_ips = set(d['vManage API IP'] for d in gns3_server_data)
    for server_ip in server_ips:
...
def vmanage_push_certs(vmanage_headers):
    server_ips = set(d['vManage API IP'] for d in gns3_server_data)
    for server_ip in server_ips:
...
def vmanage_set_device(vmanage_headers, vdevice_ip, vdevice_personality):
    server_ips = set(d['vManage API IP'] for d in gns3_server_data)
    for server_ip in server_ips:
...
def vmanage_generate_csr(vmanage_headers, vdevice_ip, vdevice_personality):
    server_ips = set(d['vManage API IP'] for d in gns3_server_data)
    for server_ip in server_ips:
...
def evpn_deploy():
    ...
    if test_lab == 1:
        ...
        for server_ip in server_ips:
            ...
            client_nodes = gns3_find_nodes_by_name(new_project_id, arista_nodes)
            abs_path = os.path.abspath(__file__)
            ...
            if client_nodes:
                for client_node in client_nodes:
                    ...
                    temp_node_name = gns3_find_nodes_by_field(new_project_id, 'node_id', 'name', client_node_id)
    ...
    if app_config.configure_mgmt_tap == 1:
        matching_nodes = gns3_find_nodes_by_field(new_project_id, 'name', 'ports', 'MGMT-Cloud-TAP')
    ...
    for server_ip in server_ips:
        ...
        if client_nodes:
            for client_node in client_nodes:
                ...
                tn.read_until(b"Copy completed successfully.")
                time.sleep(2)
                gns3_stop_node(new_project_id, client_node_id)
                gns3_start_node(new_project_id, client_node_id)
    ...
    v = 1
    leaf_nodes = gns3_find_nodes_by_name(new_project_id, "leaf")
    for i in range(1, 4):
...
def viptela_deploy():
    ...
    for i in range(1, isp_switch_count + 1):
        ...
        matching_nodes = gns3_find_nodes_by_name(new_project_id, node_name)
        if not matching_nodes:
            node_id, node_name = gns3_create_node_multi_return(new_project_id, openvswitch_template_id,
    ...
    for i in range(1, mgmt_switch_count + 1):
        ...
        matching_nodes = gns3_find_nodes_by_name(new_project_id, node_name)
        if not matching_nodes:
            node_id, node_name = gns3_create_node_multi_return(new_project_id, hub_template_id, mgmt_switch_deploy_data[
    ...
    for i in range(mgmt_switch_count):
        ...
        gns3_connect_nodes(new_project_id, mgmt_switch_node_id, 0, 0, mgmt_main_switch_node_id, 0, mgmt_switch_index)
    ...
    matching_nodes = gns3_find_nodes_by_name(new_project_id, "Cloud_ISP")
    starting_subnet = 6
    router_ip = 0
    switch_index = 0
    vedge_index = 1
    if matching_nodes:
        for matching_node in matching_nodes:
            ...
            generate_interfaces_file(isp_router_objects, router_ip, isp_switch_1_objects, isp_switch_2_objects,
                                     temp_file_name)
            router_ip += 1
            gns3_upload_file_to_node(new_project_id, node_id, "etc/network/interfaces", temp_file_name)
            vedge_index += 44
    matching_nodes = gns3_find_nodes_by_name(new_project_id, "ISP-Router")
    if matching_nodes:
        for matching_node in matching_nodes:
            temp_file_name = "ISP-Router"
            node_id = matching_node[0]
            gns3_upload_file_to_node(new_project_id, node_id, "startup-config.cfg", temp_file_name)
    # endregion
    ...
    for server_ip in server_ips:
        ...
        matching_nodes = gns3_find_nodes_by_name(new_project_id, temp_node_name)
        if matching_nodes:
            for matching_node in matching_nodes:
    ...
    for server_ip in server_ips:
        ...
        if matching_nodes:
            for matching_node in matching_nodes:
                node_id, console_port, aux = matching_node
                node_name = gns3_find_nodes_by_field(new_project_id, 'node_id', 'name', node_id)
    ...
    for server_ip in server_ips:
        ...
        if matching_nodes:
            for matching_node in matching_nodes:
                ...
                vmanage_set_org(vmanage_headers)
                vmanage_set_cert_type(vmanage_headers)
                vmanage_set_cert(vmanage_headers, vmanage_root_cert)
                vmanage_sync_rootcertchain(vmanage_headers)
                vmanage_set_vbond(vmanage_headers)
                vmanage_csr = vmanage_generate_csr(vmanage_headers, vmanage_address, 'vmanage')
                tn.write(b'exit\r\n')
                tn.read_until(b'#')
                tn.write(b'vshell\r\n')
                tn.read_until(b'$')
                ...
                vmanage_install_cert(vmanage_headers, vdevice_cert)
                vmanage_set_device(vmanage_headers, vsmart_address, "vsmart")
                vmanage_set_device(vmanage_headers, vbond_address, "vbond")
                vbond_csr = vmanage_generate_csr(vmanage_headers, vbond_address, 'vbond')
                vsmart_csr = vmanage_generate_csr(vmanage_headers, vsmart_address, 'vsmart')
                tn.write(b'exit\r\n')
                tn.read_until(b'#')
                tn.write(b'vshell\r\n')
                tn.read_until(b'$')
    ...
    for server_ip in server_ips:
        temp_node_name = f'vManage'
        matching_nodes = gns3_find_nodes_by_name(new_project_id, temp_node_name)
        vedge_nodes = gns3_find_nodes_by_name(new_project_id, "vEdge")
        if matching_nodes:
            for matching_node in matching_nodes:
                node_id, console_port, aux = matching_node
                print(f"[{util_current_time()}] - Logging in to console for node {temp_node_name}")
                for vedge_node in vedge_nodes:
                    ...
                    node_name = gns3_find_nodes_by_field(new_project_id, 'node_id', 'name', vedge_id)
                    ...
                    while True:
                        ...
                        if chassis_number and serial_number:
                            break
                        print(
                            f"[{util_current_time()}] - {node_name[0]} tried to install certificate too quickly, trying again in 10 seconds ")
                        time.sleep(10)
                    tn.write(b'exit\r\n')
                    tn.read_until(b'#')
                    ...
                    ve += 1
                    print(
                        f"[{util_current_time()}] - Completed vEdge Certificate Setup for {node_name[0]}, Remaining - {vedge_count - v}")
                    tn.close()
                    v += 1
    while True:
        try:
            ...
            time.sleep(60)
    vmanage_headers = vmanage_create_auth()
    vmanage_push_certs(vmanage_headers)
        ...
        for vedge_node in vedge_nodes:
            ...
            if v % client_every == 0:
                ...
                gns3_update_nodes(new_project_id, network_test_node_id,
                                  client_deploy_data[f"network_test_client_{v:03}_deploy_data"])
                gns3_upload_file_to_node(new_project_id, network_test_node_id, client_node_file_path, temp_file_name)
    ...
    print(
        f"[{util_current_time()}] - Waiting 5 mins to send final API call to vManage to push vEdge certificates to control devices, to resume at {util_resume_time(5)}")
    time.sleep(300)
    while True:
        try:
            auth = Authentication()
    ...
    print(
        f"[{util_current_time()}] - Waiting {wait_time} minutes to validate deployment, to resume at {util_resume_time(wait_time)}")
    time.sleep(wait_time * 60)
    server_ips = set(d['GNS3 Server'] for d in gns3_server_data)
    for server_ip in server_ips:
        ...
        matching_nodes = gns3_find_nodes_by_name(new_project_id, temp_node_name)
        client_nodes = gns3_find_nodes_by_name(new_project_id, vedge_nodes)
        client_ip = 101
        successful_site = 0
        i = 1
        if matching_nodes:
            ...
            for client_node in client_nodes:
                ...
                if "100% packet" in output:
                    client_node_name = gns3_find_nodes_by_field(new_project_id, 'node_id', 'name', client_nodes[i][0])[0]
